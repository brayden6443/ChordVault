...
import { applyEnrichmentPatch, classifyEnrichmentRows } from "../src/chords/enrichment.ts";
...
test("identical existing enrichment record is unchanged", () => {
  const result = classifyEnrichmentRows([exported()], [c]); assert.equal(result.counts.unchanged, 1); assert.deepEqual(result.rows[0]?.changedFields, []);
});

test("approved enrichment fields are classified as an update", () => {
  const result = classifyEnrichmentRows([{ ...exported(), description: "Clear and warm", moods: ["Warm"], relatedChords: ["Cmaj7"] }], [c]);
  assert.equal(result.counts.update, 1); assert.deepEqual(result.rows[0]?.changedFields.sort(), ["description", "moods", "relatedChords"]);
});

test("a protected voicing change is a conflict", () => {
  const result = classifyEnrichmentRows([{ ...exported(), fretPositions: [0, 0, 0, 0, 0, 0] }], [c]);
  assert.equal(result.counts.conflict, 1); assert.deepEqual(result.rows[0]?.changedFields, ["fretPositions"]);
});

test("applying enrichment preserves every protected field", () => {
  const row = classifyEnrichmentRows([{ ...exported(), difficulty: 4, styles: ["Indie"], description: "Updated" }], [c]).rows[0]!;
...
test("mixed batches classify new, update, unchanged, and conflict independently", () => {
  ...
  const result = classifyEnrichmentRows([fresh, { ...exported(), description: "Updated" }, exported(), { ...exported(), fretPositions: [0, 0, 0, 0, 0, 0] }], [c]);
  assert.deepEqual(result.counts, { new: 1, unchanged: 1, update: 1, conflict: 1, invalid: 0 });
});
...
test("exported CSV accepts AI enrichment and round-trips as an update", () => {
  ...
  const candidate = importRecordCandidate(row, 0); const result = classifyEnrichmentRows([candidate], [c]);
  assert.equal(result.counts.update, 1); assert.deepEqual(result.rows[0]?.changedFields.sort(), ["description", "moods", "relatedChords"]);
});
